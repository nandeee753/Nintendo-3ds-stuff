...
void enterConsoleBottom(void) {
  consoleInit(GFX_BOTTOM, NULL);
  printf("Hello World!\n");
}

void exitConsoleBottom(void) {
  C3D_Init(C3D_DEFAULT_CMDBUF_SIZE);
}

void konamiCodeFn(void) {
  if (console) { exitConsoleBottom(); }
  else { enterConsoleBottom(); }
}
...
int main()
{
  ...
  // ------------------------------------------------------------
  createInputCombination("debug mode", KONAMI_CODE, KONAMI_CODE_LENGTH, &konamiCodeFn);
  // ------------------------------------------------------------
